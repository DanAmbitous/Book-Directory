...
router.delete("/", async (req, res) => {
  try {
    ...
    if (books.length > 0) {
      res.send(
        `Deleted the following books ${bookStorer.map(
          (book) => ` ${book.title}`
        )}`
      )
    } else {
...
router.delete("/:id", getABook, async (req, res) => {
  try {
    ...
    res.status(500).json({ message: error.message + "asdf" })
  }
})
...
async function getABook(req, res, next) {
  ...
  try {
    let book = await bookSchema.findOne({ title: req.params.id })

    res.book = book

    if (book == null) {
      return res.status(404).json({
        message: `Cannot find the book by the id of ${req.params.id}`,
      })
    }
...
async function getBookByTitle(req, res, next) {
  try {
    ...
    if (book.length == 0) {
      return res.status(404).json({
        message: `Can't find a book by the username of ${req.params.title}`,
      })
    }
